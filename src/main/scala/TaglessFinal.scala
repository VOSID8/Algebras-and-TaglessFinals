...
object TaglessFinal {
    ...
    def demoTaglessFinal(): Unit = {
        ...
        val expr = or(b(true), and(b(false), b(true)))
    }
    //here we are working with the values
    //we are not working with the types
    //we get the same thing with Tagless and tagless final
}
}
